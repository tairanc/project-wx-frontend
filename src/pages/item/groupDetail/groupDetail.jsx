...
import {LoadingRound, ShareConfig, getJSApi} from 'component/common';
...
class GroupDetail extends Component {
	...
	componentDidMount() {
		...
		axios.request({...pageApi.GroupBuy, params: {object_id: objectID}}).then(({data}) => {
			if (data.code === 0) {
				...
				if (!itemFlag) { //flag  为true表示可以通用详情页的数据，不需要重新请求
				...
				let shareInfo = {
					...
					success: function () {
						// 用户确认分享后执行的回调函数
						self.setState({shareModal: false, shadyDefault: true});
					},
					cancel: function () {
						// 用户取消分享后执行的回调函数
					}
	...
	render() {
		let {dataList, recommendDataList, update, openMsg, shadyDefault, shareModal, recommend, userId} = this.state;
		...
		return (
			update ?
				<div data-page="group-detail">
					{ shady && shadyDefault && dataList.group.is_dealed === 1 ?
			...
			<div className="group-msgb">
				...
				<div className="c-cfff c-tc text">
					...
					{data.group.group_type === "ROOKIE_GROUP" ?
						<span className="c-fs19">还差<span className="c-cf44 c-fb c-fs24">{groupPeople}</span> < span
							className="c-cf44 c-fb c-fs20"> 位新人</span></span> :
	...
	render() {
		return (
			<div className="share-success c-tc">
				<button className="go-share" onClick={this.props.goShare}>继续分享</button>
				<Link to="/">
					<button className="go-home">返回首页</button>
				</Link>
	...
	render() {
		...
		return (
			<div className="goods-msg">
				<Link
					to={userId ? `/item?item_id=${ data.item.id }&user_id=${userId}` : `/item?item_id=${ data.item.id }`}>
				...
				<div className="goods-detail">
					<Link to={`/item?item_id=${ data.item.id }`}>
				...
				<div style={{width: '96%', maxWidth: 105, display: "inline-block"}}>
					<div style={{height: 105, overflow: "hidden"}}>
						<Link to={`/item?item_id=${data.item_id}`}>
							<img src={addImageSuffix(data.images[0], '_s') || "/src/img/icon/loading/default-no-item.png"}/>
						</Link>
					</div>
	...
	render() {
		...
		return (
			...
			<section className="group-purchase-btn">
				...
				<Link className="more-pintuan" to="/groupHome">

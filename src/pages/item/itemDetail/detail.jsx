...
import {LoadingRound, EmptyPage, ShareAndTotop, CustomerService} from 'component/common';
...
export let Action = createAction({
	toCart: {
		url: "/wxapi/cart-add.html",
		type: "post"
	},
	Spec: {
		url: "/wxapi/wapapi/spec.api",
		type: "get"
	},
	DelFav: {
	...
	Fav: {
		url: "/wxapi/collection-add.api",
		type: "get"
	},
	GetCoupon: {
		url: "/wxapi/getCoupon.html",
		type: "post"
	},
	Preview: {
		url: "/wxapi/rate_preview.api",
		type: "get"
	}
}, true);
...
class ScrollImage extends Component {
	...
	render() {
		return (
			<div className="swiper-slide"><img data-src={ this.props.src.replace(/\_(\w)\.jpg$/g, '_l.jpg') }
																				 alt={this.props.alt} className="swiper-lazy"/>
	...
	render() {
		...
		return (
			<div className="c-pr" data-plugin="swiper" data-page="trade-list">
				{H5_pic ? <img className="c-pa" style={{width: '120px', top: 0, left: 0, zIndex: 2}}
											 src={H5_pic}/> : (group_data && group_data.group_type === "ROOKIE_GROUP" ?
					<img className="c-pa" style={{width: "40px", top: "10px", left: "10px", zIndex: 2}}
			...
			<Modal isOpen={this.props.modal} title="服务说明" className="modal sever-pop-wrap"
				...
				<section className="container">
					<ul>
						...
						<li>
							<div className="hd">
								<i className="detail-serve-icon"></i> {!item.tags.free_refund ? "不支持" : null}七天无理由退换货
							</div>
							<div className="tax-cont">
								{item.tags.free_refund ? "本商品支持7天无理由退换货,在商品签收之日起7天内可发起退换货申请,退回商品应当完好" : "本商品不支持七天无理由退换货"}
							</div>
						</li>
				...
				<section className="promotion-list c-fl">
					<div className="list-details">
						<ul>
							...
							<li><i className="detail-serve-icon"></i>{data.item.tags.free_refund ? "支持" : "不支持"}七天无理由退换货</li>
...
export class BuyModal extends Component {
	...
	initState(num) {
		...
		let spec = Object.keys(this.props.specs);
	...
	getActionData(flag) {
		let ret = {
			...
			"item[cart_price]":getPrice({...this.props, ...this.state}),
				...
				this.isShelves() ?
					this.isSaleOut() ?
						<div className="action-btn-group c-fr">
							<BuyModal {...props} getCart={this.props.getCart}/>
				...
				<div className="buy-action">
					...
					<div className="collect serve_kf">
						<Link to="/homeIndex" className="save">
							<span className="details_kf"><img src="/src/img/icon/home-icon.png"/></span><span
							className="icon-title">首页</span>
						</Link>
					</div>
					{/*  客服 */}
					<div className="collect serve_kf">
					...
					<Collect {...this.props}/>
					{/* 购物袋 */}
					<TotopAndCart {...this.props}/>
					{/* 加入购物袋 立即购买*/}
					<Buy {...this.props} getCart={this.props.getCart}/>
				</div>
			)
		}
	}
		...
		return (
			<div className={"coupon " + CouponClassFix(itemdata)} onClick={this.getCoupon}>
				<div className="coupon-li-wrap">
					<div className="c-fl c-tc coupon-left">
					...
					<div className="c-fr coupon-right c-pr">
						...
						<p
							className="coupon-right-use-date">{(dateUtil.format(itemdata.use_start_time * 1000, "Y/M/D H:F")).slice(2)}
...
export class ActiveArea extends Component {
	render() {
		return ( <div className="active-area">
			...
			{this.props.data.item_type == "seckill" ? null :
...
export class LinkAndChange extends Component {
	...
	render() {
		let {notLogin, onTouchTap, onClick, className, style, children, isLogin, to, href} = this.props;
		let props = {
		...
		return (!(notLogin === true || isLogin) ?
			<Link {...props} onTouchTap={this.noop} onClick={this.noop} to={`/login?redirect_uri=${redirect_uri}`}>{children}</Link>
			: <Link  {...props}>{children}</Link>)
	}
}


export const LinkChange = connect(stateLogin)(LinkAndChange)
...
export class TotopAndCart extends Component {
	...
	render() {
		let countCart = this.props.cartInfo.cartInfo.countCart;
		if (countCart >= 99) {
			Popup.MsgTip({
				...
				<ul>
					<Link to='/shopCart'>
						<li className="cart"><span
							className={countCart === 0 ? "c-dpno" : (countCart >= 10 ? "width2" : "width1")}>{countCart}</span></li>
					</Link>
...
export default class detail extends Component {
	...
	render() {
		let {data} = this.props;
		return (
			<div data-page="item-details" id="itemDetails" ref="detail">

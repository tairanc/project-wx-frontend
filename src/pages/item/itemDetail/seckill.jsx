...
import {Modal, Fix,PopupModal, ModalTips} from 'component/modal';
    ...
    render() {
        ...
        return (
            <div className="buy-option-btn">
                {  getStore() ?
                    active === "cart" ?
                    <div className="btn-addcart" onTouchTap={onClickCart}>加入购物袋</div> :
                        <div>
                         { isPurchaseLimit ? <span className="purchase-limit">抱歉，海外直邮类商品和跨境保税类商品总价超过限额￥2000，请分次购买。</span>: null}
                         <div className={isPurchaseLimit ? "c-bgc9" : "" + " btn-tobuy "} onTouchTap={isPurchaseLimit ? this.noop : onClickBuy}>确定</div>
                        </div>
...
class BuyModal extends Component {
  ...
  loadCaptchas = () => {
    let self = this;
    $.ajax({
      url: "/wxapi/loadCaptcha.api?t=" + (new Date()).getTime(), // 加随机数防止缓存
      type: "get",
      dataType: "json",
...
class PromoteWrap extends Component {
    ...
    createMsg() {
        return <div className="seckill-code-content">
           ...
           <div className="code-input-area">
               <input type="text" value={this.props.inputVal} onChange={(e) => this.getInput(e)}/><img  className="get-code" src={this.props.valid.primary_image}  onClick={()=>{

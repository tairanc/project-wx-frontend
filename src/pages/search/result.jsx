...
const pageApi = {
	search:{ url:"/wxapi/search.api" , method:"post"}
};
...
//state数据到请求数据
function stateToSearch( searchData, preProperty, couponId, searchType, shop ) {
	...
	if( searchData.priceRange.min !=="" && searchData.priceRange.max !=="" ){
...
export class SearchResult extends Component {
	...
	dropDown=( me )=>{
		let { searchData, dispatch, totalPage, preProperty, couponId, searchType, shop, listLoad, dataList } = this.props;
		...
		axios.request({
			...pageApi.search,
			data: JSON.stringify( stateToSearch( searchData, preProperty, couponId, searchType, shop ) ),
	...
	navClick =( type, direct )=>{
		...
		if( type === orderBy.type ){
			if( type === "price" ){
				dispatch( createActions('sortOrder', { navType:type, direct: orderBy.direct==="asc"?"des":"asc" } ));
			}else{
				return false;
			}
		}else{
	...
	updateResult = () => {
		let {dispatch, searchData, preProperty, couponId, searchType, shop } = this.props;
	...
	searchBarSearch=()=>{
		let { keyWord, searchType, dispatch } = this.props;
		if( keyWord ===""){
...
//列表内容控制
class ListInfoCtrl extends Component{
	render(){
...
class OneItem extends Component{
	render(){
		const { H5_pic,group_type,item_id,sell_price,market_price,support_ecard,trade_type, primary_image, title,promotionDetail,activity_price,activity_store,store,valid } = this.props.data;
		let saleOut = null;
		activity_store !== undefined ?  saleOut = activity_store<=0 : saleOut = store <=0;
		if( !valid  ) saleOut = true;
		return(
			<div className="one-item-grid">
				<div className="one-item"   onClick={ this.props.toItem.bind( null,`/item?item_id=${item_id}`) }>
				...
				{(storeHomeRequest||storeHome)?"":(
					<div className="grid-right">
						...
						<span className={`nav-span${this.props.filter?" active":""}`}>
							<span>筛选</span>
						<i className={ this.props.filter?"arrow-btm-m-r-icon":"arrow-btm-m-icon"}> </i>
						</span>
						</div>
					</div>
				)}
...
function listFilterDispatch( dispatch, props ) {
	return {
		...
		filterPrice( ctrl,e ){
			dispatch( createActions('filterPrice', { ctrl, value: e.target.value ===""?"":+e.target.value }));
		},
		minPrice:( e )=>{
...
export class ListFilterMain extends Component{
	...
	getListRight=()=>{
		...
		switch ( listType ){
			case "brand":
				result = listData.list.map( (item,j)=>{
					return <div key={j}
...
export function searchResultDispatch( dispatch, props ) {
	let { key,coupon_id, brand, category, property,type, shop } = props.location.query;

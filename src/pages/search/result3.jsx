...
class SearchResult extends Component {
	...
	componentWillMount() {
		...
		let initSevice = Object.assign({},this.props.searchData.service);//获取初始化service参数 解决从详情页返回数据不一致刷新
		...
		let searchData = {
			...
			service: service !== undefined ? this.formatSearchData(service,initSevice) : "",
			page: 1,
	...
	changeUrl(searchData) {
		...
		for (let key in searchData) {
			if (searchData.hasOwnProperty(key)) {
				if (searchData[key]) {
					searchStr += "&" + apiToUrl[key] + "=" + (key == 'service' ? this.formatSearchData(searchData[key],initSevice) : searchData[key]);
				}
			}
		}
		history.replaceState({}, null, href + "?" + searchStr.substr(1));
	}
	formatSearchData(data,initSevice) {
		let str = "";
		//let obj = {};
		if (typeof data == 'object') {
			...
			for(let j in initSevice){
                for (let i = 0; i < array.length; i++) {
                    initSevice[array[i]] = 1;
                }
			}
			return initSevice;
		}
...
class QuickFilter extends Component {
	...
	filterClick(type, style) {
		if (style == 'click') {
            ...
            let service = Object.assign({},this.props.searchData.service);

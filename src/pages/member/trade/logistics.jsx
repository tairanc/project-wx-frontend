...
import {WXAPI} from 'src/config/index'
import axios from 'js/util/axios'
import './logistics.scss';
import {addImageSuffix} from "js/util/index";

const ctrlAPI = {
    packageInfo: {url: `${WXAPI}/user/shopOrderPackage/info`, method: "get"},   //包裹信息
    tracking: {url: `${WXAPI}/user/orderTracking/get`, method: "get"} //物流数据
};
...
class PackageGoods extends Component {
    ...
    render() {
        ...
        let goods = data.map(function (item, i) {
            return <div className="goods-img c-fl c-mb15 swiper-slide" key={i}><Link
                to={`/item?item_id=${item.item_id}`}><img
                src={item.primary_image?addImageSuffix(item.primary_image, "_s"): require('../../../img/icon/loading/default-no-item.png')}/></Link><span>x{item.packed_num}</span>
            </div>
        });
        return (
            <div className="package-goods">
                <div className={"c-clrfix package-imgs swiper-container " + this.props.className}
                     style={{'width': '100%'}}>
                    <div className="swiper-wrapper">
                        {goods}
                    </div>
